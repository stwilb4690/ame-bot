...
impl AtomicMarketState {
    ...
    /// Check for Kalshi same-platform opportunities.
    ///
    /// - Same-platform arb: Kalshi YES ask + Kalshi NO ask < 100¢ (profit buying both sides).
    /// - Wide spread: round-trip cost (yes_ask + no_ask - 100) exceeds `wide_spread_threshold`.
    pub fn check_kalshi_opps(&self, wide_spread_threshold: u16) -> KalshiOpps {
        let (k_yes, k_no, _, _) = self.kalshi.load();
        if k_yes == NO_PRICE || k_no == NO_PRICE {
            return KalshiOpps::default();
        }

        let mut opps = KalshiOpps::default();
        let total = k_yes as i32 + k_no as i32;
        ...
        if total < 100 {
            ...
            let spread = (total - 100) as u16;
            if spread >= wide_spread_threshold {
                opps.wide_spread = Some(WideSpread {
                    yes_ask: k_yes,
                    no_ask: k_no,
                    spread_cents: spread,
                });
...
/// Build a fee table for a given rate.
///
/// fee_cents = ceil(rate × P × (1-P) × 100)
/// Using integer math: ceil(rate_bps × p × (100-p) / 10_000_000)
/// where rate_bps = round(rate × 100_000)
fn build_fee_table(rate: f64) -> [u16; 101] {
    let mut table = [0u16; 101];
    if rate <= 0.0 {
        return table;
    }
    // rate_times_100 = round(rate × 100), e.g. 7 for 0.07
    let rate_times_100 = (rate * 100.0).round() as u64;
    for p in 1u64..100 {
        // fee = ceil(rate_times_100 × p × (100-p) / 10000)
        let numerator = rate_times_100 * p * (100 - p);
        table[p as usize] = ((numerator + 9999) / 10000) as u16;
    }
    table
}
...
/// A Kalshi same-platform arbitrage: YES ask + NO ask < 100¢ (buy both sides for profit).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SamePlatformArb {
    pub yes_ask: u16,
    pub no_ask: u16,
    /// Profit after fees (may be negative if fees eat the spread).
    pub profit_cents: i16,
}

/// A wide bid-ask spread on a Kalshi market.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WideSpread {
    pub yes_ask: u16,
    pub no_ask: u16,
    /// Round-trip cost above $1.00 in cents (yes_ask + no_ask - 100).
    pub spread_cents: u16,
}

/// Results of a Kalshi same-platform opportunity scan for one market.
#[derive(Debug, Default)]
pub struct KalshiOpps {
    pub same_platform: Option<SamePlatformArb>,
    pub wide_spread: Option<WideSpread>,
}
...
impl GlobalState {
    ...
    /// Add a Kalshi-only market (no Polymarket pair), returns market_id.
    ///
    /// These markets are subscribed to for price monitoring and same-platform
    /// arb detection, but are never paired with a Polymarket token.
    pub fn add_kalshi_only(&mut self, ticker: &str, description: &str) -> Option<u16> {
        if self.next_market_id as usize >= MAX_MARKETS {
            return None;
        }
        let market_id = self.next_market_id;
        ...
        let hash = fxhash_str(ticker);
        self.kalshi_to_id.insert(hash, market_id);
        self.kalshi_descriptions.insert(hash, description.into());
        // pair stays None for Kalshi-only markets

        Some(market_id)
    }

    /// Get the description for a Kalshi market (works for both paired and Kalshi-only markets).
    pub fn get_kalshi_description(&self, ticker: &str) -> Option<&str> {
        let hash = fxhash_str(ticker);
        // Check Kalshi-only descriptions first
        if let Some(desc) = self.kalshi_descriptions.get(&hash) {
            return Some(desc);
        }
        // Fall back to pair description
        let id = *self.kalshi_to_id.get(&hash)?;
        self.markets[id as usize].pair.as_ref().map(|p| &*p.description)
    }
